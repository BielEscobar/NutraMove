...
## Evolução da anamnese, reavaliação, fotos e prompt de IA v2

- Backend: suíte completa com **427 testes aprovados**, nenhum pulado e um aviso interno conhecido de Starlette/AnyIO, em 7m17s. O recorte de IA, anamnese e fotos teve 30 testes aprovados. Os testes usaram PostgreSQL em schemas descartáveis e não fizeram chamadas reais à OpenAI.
- Migration `20260918_11`: upgrade, downgrade, novo upgrade e `alembic check` aprovados nos schemas de teste. O banco local está em `20260918_11 (head)` e o check final não encontrou operações novas. Uma divergência de comprimento de colunas enum criada por uma versão intermediária local foi alinhada com `ALTER TABLE`, sem remover registros ou tabelas.
- Qualidade backend: Ruff check, Ruff format --check, mypy strict em 128 arquivos e `pip check` aprovados.
- Qualidade frontend: Biome em 134 arquivos, TypeScript e build de produção do Next.js aprovados.
- Infra: configurações Compose local e de produção aprovadas. As imagens de produção de backend e frontend foram reconstruídas; o backend cria o diretório privado com usuário sem privilégios, e os dois stacks usam volume nomeado para as fotos.
- Segurança: testes cobrem validação condicional da anamnese, upload atômico, assinatura/MIME/tamanho de imagem, isolamento Student/Professional e ausência de chave/caminho de armazenamento nas respostas. MASTER não recebeu rota para fotos. O contexto de IA não inclui imagens, chaves de armazenamento nem identificadores pessoais.
- A tentativa de gerar capturas Chrome em 1366/1024/768/375 px não produziu arquivos no ambiente atual. Portanto, o build e as verificações estáticas passaram, mas a nova interface de anamnese, reavaliação e comparação de fotos ainda requer inspeção visual manual nessas larguras.
- Nenhum dado real, segredo, chamada real à OpenAI, commit, push, merge, tag ou deploy foi realizado.

## Homologação final da anamnese, fotos e reavaliação

- Suíte backend final: **431 testes aprovados, zero pulados e zero falhas**, em 10m28s. Permanece um aviso interno conhecido de Starlette/AnyIO. O recorte de anamnese/fotos/IA passou com 22 testes.
- Os novos testes verificam a transferência A→B na listagem e no download binário, a manutenção do acesso do Student, a rejeição atômica quando SIDE é inválida e a limpeza do primeiro arquivo se a gravação de SIDE falhar. O contexto Diet/Workout usa o snapshot COMPLETED, ignora o PENDING mais novo e exclui dados iniciais substituídos, identificadores pessoais e referências de foto.
- Ruff check e format --check aprovados; mypy strict aprovou 128 arquivos; `pip check` sem incompatibilidades. Biome aprovou 134 arquivos; TypeScript e Next production build passaram. Compose local e de produção passaram. As imagens Docker finais de backend e frontend foram construídas com valores sintéticos.
- Alembic confirmou uma única head `20260918_11`; `current` do backend local está em `20260918_11 (head)` e `check` retornou `No new upgrade operations detected`. Nenhuma migration nova foi necessária.
- **VISUAL AUTOMATED VALIDATION = NOT AVAILABLE.** O Chrome abriu `/register` com HTTP 200, mas duas tentativas de screenshot, inclusive com perfil isolado, não produziram arquivo. Não houve inspeção visual válida das novas telas em 1366×768, 1024×768, 768×1024 ou 375×812. Validar manualmente cadastro completo, upload FRONT/SIDE, evolução Student, ficha Professional, reavaliação em seis etapas, overflow, acentuação, navegação, proporção das imagens e controles mobile.
- `infra/.env` continua ignorado pelo Git; varredura dos arquivos alterados encontrou apenas credenciais fictícias de teste. `git diff --check` passou. Não houve chamada real OpenAI, deploy ou operação de escrita no Git.
- Homologação corretiva de IA/rotas/textos: suíte backend completa com **434 testes aprovados**, zero falhas/pulados e um aviso interno conhecido de Starlette/AnyIO. Testes focados de IA (30) e testes de rotas do frontend (5) passaram com provider falso. Ruff check/format, mypy strict (128 arquivos), pip check, Biome (135 arquivos), TypeScript, build Next, Compose local/produção e Alembic `20260918_11`/`check` passaram. O build atual respondeu HTTP 200 para ficha Professional e suas quatro subrotas de dieta, treino, evolução e hidratação em `localhost:3000`; a instância anterior atendia as subrotas com 404 apesar de elas existirem no App Router. Nenhuma chamada real à OpenAI foi feita nesta validação.
- Ajuste final de publicação do Workout: **445 testes backend passaram**, zero falhas/pulados, um aviso interno Starlette/AnyIO. A regressão cobre aprovação direta do Workout AI sem edição, leitura Student após publicação, treino manual com descanso e bloqueio de conteúdo vazio/inválido para ambas as origens. Ruff check/format, mypy strict (128 arquivos), pip check, Biome (135 arquivos), TypeScript, Next build e Compose local/produção passaram. Alembic permaneceu em `20260918_11 (head)` e `check` não detectou operações novas. Nenhuma chamada real OpenAI foi feita.
- Descanso estrutural no Workout: **448 testes backend passaram**, zero falhas/pulados; um aviso interno Starlette/AnyIO. Testes cobrem `isRest` no schema da IA, geração fake, persistência, GET, aprovação direta sem edição, leitura Student, criação manual, títulos independentes do tipo de dia, dias ativos vazios e planos apenas de descanso. A suíte executou upgrade/downgrade/upgrade e `alembic check` em schemas temporários. Ruff check/format, mypy strict (129 arquivos), pip check, Biome (135 arquivos), TypeScript, Next build e Compose local/produção passaram. Alembic local foi atualizado para `20260920_12 (head)` e `check` não detectou operações novas. Nenhuma chamada real OpenAI foi feita.
- Correção final Workout e reavaliação: **454 testes backend passaram**, zero falhas/pulados e um aviso interno Starlette/AnyIO. Testes novos cobrem 4/5/6 dias ativos sem descanso explícito, rejeição de 7 dias ativos, aprovação sem PATCH, contexto PENDING/IN_REVIEW/COMPLETED, geração de versões 2 e preservação do histórico. Frontend: 5 testes de rotas e 2 testes do estado inicial do rascunho passaram; Biome (137 arquivos), TypeScript e build Next passaram. Ruff check/format, mypy strict (129 arquivos), pip check, Compose local/produção e Alembic `20260920_12 (head)`/`check` passaram. Nenhuma migration nova ou chamada real OpenAI foi feita nesta correção.

## Homologação corretiva: aprovação de Workout e exclusão de rascunhos (20/09/2026)

A inspeção somente estrutural do PostgreSQL local encontrou um Workout AI PENDING_REVIEW com quatro dias: 6, 6, 7 e 0 exercícios; todos tinham `is_rest=false`. ORM em sessão independente e schema da API preservaram os mesmos valores. A regra de publicação corretamente recusa o quarto dia ativo vazio. O registro não foi alterado automaticamente. O fluxo novo foi testado com fake provider, commit PostgreSQL, sessões independentes, três dias ativos com três exercícios cada, GET, aprovação sem PATCH e leitura Student. Exclusão transacional de versões DRAFT/PENDING_REVIEW de Diet/Workout, proteção de APPROVED/ARCHIVED, carteira, cascata, preservação das demais versões e regeneração seletiva passaram.

Suíte backend final: `469 passed`, `0 failed`, `0 skipped`, um aviso interno Starlette/AnyIO. Ruff check/format, mypy strict (116 arquivos), pip check, Biome (137 arquivos), TypeScript, rotas frontend e Next build passaram. Compose local/prod `config --quiet` passou com env sintético; Alembic `heads/current` = `20260920_12` e `check` sem divergências. Nenhuma migration ou chamada OpenAI real. O rebuild Next exigiu limpar apenas o cache gerado `.next`/`.next/dev` bloqueado/corrompido pelo OneDrive; o build final passou.
